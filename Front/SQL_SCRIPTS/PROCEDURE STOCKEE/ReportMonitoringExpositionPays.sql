...
ALTER PROCEDURE ReportMonitoringExpositionPays
...
insert #GroupeCalcule values ('EXTERNE',@maxGroup) 
  ...
  --Zone USA + Australie
   insert into #pays_zone values (6,1,'US','USA + Canada')
   insert into #pays_zone values (6,2,'CA','USA + Canada')

  --Zone USA + Australie
   insert into #pays_zone values (7,1,'AU','Australie+Japon')
   insert into #pays_zone values (7,2,'JP','Australie+Japon')
   
   --Zone 3 : Autres pays (inclure Pays is null)
   insert into #pays_zone 
   select 8, row_number()  over (order by pays.libelle), pays.id ,'Reste' 
   from  [PAYS] as pays
