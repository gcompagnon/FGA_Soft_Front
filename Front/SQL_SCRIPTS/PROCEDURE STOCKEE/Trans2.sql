...
--DROP PROCEDURE Trans2
ALTER PROCEDURE Trans2
       @date DATETIME
AS 
...
SELECT ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
       pp.isin_titre AS 'Isin_proxy_Niv_2',--le code du fond à la source
...
SELECT  ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
	   pp.code_proxy AS 'Isin_proxy_Niv_2',
	   Groupe,Dateinventaire,ma.Compte,ma.ISIN_Ptf,ma.Libelle_Ptf,
...
SELECT ntt.Groupe,ntt.Dateinventaire,ntt.Compte,ntt.ISIN_Ptf,ntt.Libelle_Ptf,ntt.code_Titre,ntt.isin_titre,ntt.Libelle_Titre,ntt.Valeur_Boursiere,
	   ...
	   ntt.pct_det_Niv_0,ntt.Isin_origine_Niv_1,
	   CASE WHEN ntt.code_Titre='Cash Mandat' THEN 1 ELSE ntt.pct_det_Niv_1 END AS 'pct_det_Niv_1',
	   ntt.Isin_proxy_Niv_1,		   
...
SELECT po.Groupe,po.Dateinventaire,po.Compte,po.ISIN_Ptf,po.Libelle_Ptf,
	   ...
	   po.pct_det_Niv_1 * fga.Valeur_Boursiere As 'Valeur_Boursiere',
	   po.pct_det_Niv_1 * fga.Coupon_Couru AS 'Coupon_Couru',
	   po.pct_det_Niv_1 * fga.Valeur_Comptable AS 'Valeur_Comptable',
	   po.pct_det_Niv_1 * fga.Coupon_Couru_Comptable AS 'Coupon_Couru_Comptable',
	   po.pct_det_Niv_1 * fga.PMV AS 'PMV',
	   po.pct_det_Niv_1 * fga.PMV_CC AS 'PMV_CC',
	   ...
	   po.pct_det_Niv_1 * fga.quantite AS quantite,
	   ...
	   isnull(po.Isin_proxy_Niv_2,po.isin_titre) AS 'Isin_origine_Niv_2',  -- isin du fonds source (si proxy.OPCVM, c est le fonds initial)
	   CASE WHEN an.AN > 0 THEN po.pct_det_Niv_1 * ( fga.Valeur_Boursiere + fga.Coupon_Couru) / an.AN-- poids du fonds Source dans le mandat si c'est un fond
					  ELSE NULL END AS 'pct_det_Niv_2',  -- si ce n'est pas un fonds FGA : vide
       CASE WHEN po.Isin_proxy_Niv_2 is not null THEN po.isin_titre
                                                 ELSE NULL END AS 'Isin_proxy_Niv_2',   -- isin du fonds utilisé en proxy (OPCVM FGA)	   
  	   '' AS 'Isin_origine_Niv_3', NULL AS 'pct_det_Niv_3', NULL AS 'Isin_proxy_Niv_3',	   
	   2 AS 'Numero_Niveau',
	   CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
	   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
...
INTO #Lignes_OPCVM  -- niveau 2
FROM  #OPCVM_FGA po -- niveau 1
...
CASE WHEN p.code_Titre = 'Liquidité(OPCVM)'  THEN 'Cash OPCVM' ELSE p.code_Titre END AS 'isin_titre',p.Libelle_Titre,
		...
		left(po.isin_titre,12) AS 'Isin_origine_Niv_2', -- isin du fonds source
		...
		ELSE NULL END AS 'pct_det_Niv_2',  -- si ce n'est pas un fonds FGA : vide        
		left(po.Isin_proxy_Niv_2,12) AS 'Isin_proxy_Niv_2',   -- code du proxy utilisé
  	   '' AS 'Isin_origine_Niv_3', NULL AS 'pct_det_Niv_3', NULL AS 'Isin_proxy_Niv_3',		
		2 AS 'Numero_Niveau',
		CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
			   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
...
INTO #Lignes_PROXY --Niveau2
FROM  #PROXY po  -- niveau 1
	LEFT OUTER JOIN PTF_PROXY p ON p.code_proxy = po.Isin_proxy_Niv_2 AND p.Date= @date
