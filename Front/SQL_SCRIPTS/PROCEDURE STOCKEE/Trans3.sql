...
--DROP PROCEDURE Trans3
ALTER PROCEDURE Trans3
       @date DATETIME
AS 
...
SELECT ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
       ...
       pp.isin_titre AS 'Isin_proxy_Niv_3',--le code du fond à la source       
...
SELECT  ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
       ma.Isin_proxy_Niv_2,ma.Isin_origine_Niv_2,ma.pct_det_Niv_2,
	   pp.code_proxy AS 'Isin_proxy_Niv_3',
	   Groupe,Dateinventaire,ma.Compte,ma.ISIN_Ptf,ma.Libelle_Ptf,
...
SELECT ntt.Groupe,ntt.Dateinventaire,ntt.Compte,ntt.ISIN_Ptf,ntt.Libelle_Ptf,ntt.code_Titre,ntt.isin_titre,ntt.Libelle_Titre,ntt.Valeur_Boursiere,
	   ...
	   ntt.Isin_origine_Niv_2,
	   CASE WHEN ntt.code_Titre='Cash Mandat' THEN 1 ELSE ntt.pct_det_Niv_2 END AS 'pct_det_Niv_2',
	   ntt.Isin_proxy_Niv_2,
...
SELECT po.Groupe,po.Dateinventaire,po.Compte,po.ISIN_Ptf,po.Libelle_Ptf,
	   ...
	   po.pct_det_Niv_2 * fga.Valeur_Boursiere As 'Valeur_Boursiere',
	   po.pct_det_Niv_2 * fga.Coupon_Couru AS 'Coupon_Couru',
	   po.pct_det_Niv_2 * fga.Valeur_Comptable AS 'Valeur_Comptable',
	   po.pct_det_Niv_2 * fga.Coupon_Couru_Comptable AS 'Coupon_Couru_Comptable',
	   po.pct_det_Niv_2 * fga.PMV AS 'PMV',
	   po.pct_det_Niv_2 * fga.PMV_CC AS 'PMV_CC',
	   ...
	   po.pct_det_Niv_2 * fga.quantite AS quantite,
	   ...
	   po.Isin_origine_Niv_2, po.pct_det_Niv_2, Isin_proxy_Niv_2,
	   isnull(po.Isin_proxy_Niv_3,po.isin_titre) AS 'Isin_origine_Niv_3',  -- isin du fonds source (si proxy.OPCVM, c est le fonds initial)
	   CASE WHEN an.AN > 0 THEN po.pct_det_Niv_1 * ( fga.Valeur_Boursiere + fga.Coupon_Couru) / an.AN-- poids du fonds Source dans le mandat si c'est un fond
					  ELSE NULL END AS 'pct_det_Niv_3',  -- si ce n'est pas un fonds FGA : vide
       CASE WHEN po.Isin_proxy_Niv_3 is not null THEN po.isin_titre
                                                 ELSE NULL END AS 'Isin_proxy_Niv_3',   -- isin du fonds utilisé en proxy (OPCVM FGA)	   
	   3 AS 'Numero_Niveau',
	   CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
	   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
			WHEN ta.Types IS NOT NULL AND ta.Types <> 'AS_PTF' THEN ta.Types -- transco pour les actifs sauf cash et monétaire
...
INTO #Lignes_OPCVM  -- niveau 3
FROM  #OPCVM_FGA po -- niveau 2
...
CASE WHEN p.code_Titre = 'Liquidité(OPCVM)'  THEN 'Cash OPCVM' ELSE p.code_Titre END AS 'isin_titre',p.Libelle_Titre,
		...
		left(po.isin_titre,12) AS 'Isin_origine_Niv_3', -- isin du fonds source
		...
		ELSE NULL END AS 'pct_det_Niv_3',  -- si ce n'est pas un fonds FGA : vide        
		left(po.Isin_proxy_Niv_3,12) AS 'Isin_proxy_Niv_3',   -- code du proxy utilisé
		3 AS 'Numero_Niveau',
		CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
	   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
			WHEN ta.Types IS NOT NULL AND ta.Types <> 'AS_PTF' THEN ta.Types -- transco pour les actifs sauf cash et monétaire
...
INTO #Lignes_PROXY --Niveau3
FROM  #PROXY po  -- niveau 2
	LEFT OUTER JOIN PTF_PROXY p ON p.code_proxy = po.Isin_proxy_Niv_3 AND p.Date= @date
