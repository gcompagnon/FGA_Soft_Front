...
VUE en mode DURATION¨PONDEREE : chaque sous-rubrique est le pondération des durations dans la rubrique
...
--GO
-- execute as fga
-- DROP PROCEDURE ReportMonitoringExpositionPaysDur

ALTER PROCEDURE ReportMonitoringExpositionPaysDur
...
BEGIN
  set @cleReport = 'MonitorExpoPaysDur'
END
...
/*       Pour la duration: aucun sens de l'afficher sur les compartiments, actions, 
insert into #rapport_pays 	  
...
/************************************************************************/
 ...
 create table #duration(groupe varchar (25)COLLATE DATABASE_DEFAULT ,
                                    pays varchar(30)COLLATE DATABASE_DEFAULT ,
                                    libelle varchar(60)COLLATE DATABASE_DEFAULT ,
                                    valeur float)
                                    
                                    
 insert into #duration (groupe, pays, libelle, valeur)
 select  rapport.Groupe as groupe,		 
		 rapport.pays as pays,
		 rapport.libelle as sousSecteur,
         SUM( ( Valeur_Boursiere + Coupon_Couru ) * Duration ) as valeur
  from (select * from #rapport_pays, #Groupe) as rapport 
  left outer join [dbo].[PTF_TRANSPARISE] as tr 
  on tr.Groupe = rapport.Groupe
  and tr.Dateinventaire = @dateInventaire
...
/*********************************************************/
  ...
  left outer join #duration as rapport on rapport.groupe IN (select composant.groupeComposant from #GroupeCalculeComposant as composant where composant.groupeCalcule = gr.groupe)
...
-- insertion  des données : nominateur / encours
 insert 
  ...
  select @cleReport as cle,
		 ...
		 d.valeur/e.valeur-- si les durations ne sont pas renseignées : le résultat est nul, et la moyenne globale peut paraitre incorrecte
		 end as valeur
  from #encours as e
  left outer join #duration as d on d.groupe = e.groupe and d.pays = e.Pays and d.libelle = e.libelle         
      
   /* 
select e.*,d.*, d.valeur as 'duration ponderee nominateur', d.valeur/e.valeur from #encours as e
left outer join #duration as d on d.groupe = e.groupe and d.pays = e.Pays and d.libelle = e.libelle
order by e.OrdreGroupe, e.ordrePays, e.ordre 
*/
--select * from #duration where Pays = 'NON ATTRIBUE' order by groupe
--select * from #encours where Pays = 'NON ATTRIBUE'  order by groupe
--select * from #encours  order by ordregroupe, ordrePays, ordre
--select * from #duration where ( Groupe like 'MM A%' or Groupe = 'Retraite' ) and Pays like '%France%' order by Groupe, libelle
/*
select isin_titre, Valeur_Boursiere , Coupon_Couru  , Duration,(Valeur_Boursiere + Coupon_Couru ) * Duration  from PTF_TRANSPARISE as tr 
 where Groupe = 'MMP'
  and Dateinventaire = '31/08/2011'
  and Numero_Niveau = 2
  and  secteur in (select libelle from #secteurObligations)
--  and sous_secteur in (select libelle from [SOUS_SECTEUR] as secteur where secteur.id_secteur like 'A %' or  secteur.id_secteur = ( 'F ACTIONS') )
--  and Pays = 'Finlande'
--  and ( tr.Pays = 'Not Available' or  tr.Pays is null  or Sous_Secteur is null)  
  
  select * from #encours where Groupe = 'MM AGIRC'
select * from  #duration where Groupe = 'MM AGIRC'
select * from #encours as e
  left outer join #duration as d on d.groupe = e.groupe and d.pays = e.Pays and d.libelle = e.libelle    
 where e.Pays = 'NON ATTRIBUE' order by e.ordreGroupe, e.ordrePays,e.ordre
 */
--select * from #encours where Pays = 'NON ATTRIBUE'  order by groupe


--select * from PTF_RAPPORT_NIV2  where date= '07/09/2011' and cle = 'MonitorExpoPaysDur' order by classementRubrique, classementSousRubrique
