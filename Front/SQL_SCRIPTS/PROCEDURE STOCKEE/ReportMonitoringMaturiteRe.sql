...
/****** Object:  StoredProcedure [dbo].[ReportMonitoringMaturiteRe]    Script Date: 09/18/2012 10:09:15 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[ReportMonitoringMaturiteRe] 
...
BEGIN
  set @cleReport = 'MonitoringMaturiteRe'
END
...
create table #rendement(groupe varchar (25) COLLATE DATABASE_DEFAULT, maturite varchar(15) COLLATE DATABASE_DEFAULT, rendement float)
insert into #rendement
SELECT gm.groupe as groupe,
		gm.maturite as maturite,
		SUM((valeur_boursiere+coupon_couru)*rendement) as rendement
	FROM #groupe_maturite as gm
	left outer join [dbo].[PTF_TRANSPARISE] as tr 
	on tr.Groupe = gm.Groupe
	and tr.Dateinventaire = @dateInventaire
...
SELECT @cleReport as cle,
		...
		r.rendement/e.encours as valeur
	FROM #groupe_maturite as gm
	join #rendement as r
		ON r.groupe=gm.groupe AND r.maturite = gm.maturite
	join #encours as e
...
SELECT @cleReport as cle,
		...
		gm.maturite as libelle,
		gm.ordre as classementRubrique,
		SUM(r.rendement)/SUM(e.encours) as valeur
	FROM #groupe_maturite as gm
	join #rendement as r
		ON r.groupe=gm.groupe AND r.maturite = gm.maturite
	join #encours as e
