...
--GO
-- --execute as fga
-- DROP PROCEDURE ReportConcentrationGrpEmetteurs


ALTER PROCEDURE ReportConcentrationGrpEmetteurs 
        @dateInventaire  datetime ,
        @niveauInventaire tinyint ,
		@limiteEncoursGrpEmetteurs as float OUTPUT,
...
-- construction de la table de décomposition avec le traitement particulier des Corporates Fi:
	...
	create table #Seniorite  (categorie varchar(25) COLLATE DATABASE_DEFAULT , sousSecteur varchar(60) COLLATE DATABASE_DEFAULT , ordre float)
...
--select distinct Sous_Secteur from PTF_FGA where DateInventaire = '31/08/2012'
...
IF @limiteEncoursGrpEmetteurs  IS NULL
BEGIN
...
insert into #groupe_type (groupe, ordreGroupe, rubrique, ordreRubrique, libelle, code)  
  ...
  select  tr.grp_emetteur as 'emetteur' , SUM( Valeur_Boursiere + Coupon_Couru ) as encours 
  ...
  where tr.Dateinventaire = @dateInventaire
        ...
        and   grp_emetteur not in (select libelle from PAYS ) 
        and  grp_emetteur not in ( 'PAYS BAS' ,'GRECE')        -- TODO : à supprimer après correction dans la base 
  group by grp_emetteur
  
  -- on ne prends que les emtteurs à encours > 60 Millions
  select emetteur, encours,ROW_NUMBER() OVER (ORDER BY encours desc) as ordre
  into #perimetre_emetteur 
  from #groupe_emetteur  as e
  where encours >   @limiteEncoursGrpEmetteurs 
  
  -- select * from #perimetre_emetteur 
    
  -- select * from #groupe_emetteur   
...
-- ligne global par groupe    
  ...
  and tr.Dateinventaire = @dateInventaire  
  and tr.Numero_Niveau = @niveauInventaire      
  and tr.Grp_Emetteur = eg.emetteur
  ...
  and tr.Dateinventaire = @dateInventaire  
  and tr.Numero_Niveau = @niveauInventaire      
  and tr.Grp_Emetteur = em_sous_secteur.emetteur 
...
/**********************************************/
  ...
  where gr.groupe IN ('ASSURANCE', 'RETRAITE', 'EXTERNE', 'FGA')
...
-- calcul des colonnes POIDS RETRAITE et POIDS ASSURANCE
  ...
  select @rapportCle as cle,
         ...
         (montantRubrique.valeur / NULLIF(encoursTotal.valeur,0) ) as valeur
  from #GroupeCalcule as gr,
  ...
  where  gr.groupe IN ('POIDS Retraite',  'POIDS Assurance','POIDS Externe', 'POIDS FGA') 
