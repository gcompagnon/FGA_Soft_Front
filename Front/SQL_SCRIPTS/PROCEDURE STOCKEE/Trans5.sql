...
--DROP PROCEDURE Trans5
ALTER PROCEDURE Trans5
       @date DATETIME
AS 
...
CREATE TABLE #OPCVM_FGA([pct_det_Niv_0] [float] NULL, [Isin_proxy_Niv_1] [varchar](12) COLLATE DATABASE_DEFAULT NULL,
	...
	[Isin_origine_Niv_3] [varchar](12) COLLATE DATABASE_DEFAULT NULL,[pct_det_Niv_3] [float] NULL,[Isin_proxy_Niv_5] [varchar](12) COLLATE DATABASE_DEFAULT NULL,
...
SELECT ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
       ...
       pp.isin_titre AS 'Isin_proxy_Niv_5',--le code du fond à la source       
...
SELECT  ma.pct_det_Niv_0,ma.Isin_proxy_Niv_1,ma.Isin_origine_Niv_1,ma.pct_det_Niv_1,
       ma.Isin_proxy_Niv_2,ma.Isin_origine_Niv_2,ma.pct_det_Niv_2,
	   ma.Isin_proxy_Niv_3,ma.Isin_origine_Niv_3,ma.pct_det_Niv_3,
	   pp.code_proxy AS 'Isin_proxy_Niv_5',
	   Groupe,Dateinventaire,ma.Compte,ma.ISIN_Ptf,ma.Libelle_Ptf,
...
CREATE TABLE #NO_TRANS(	[Groupe] [varchar](25) NULL,[Dateinventaire] [datetime] NOT NULL,
	...
	[Isin_origine_Niv_5] [varchar](12) NULL,[pct_det_Niv_5] [float] NULL,[Isin_proxy_Niv_5] [varchar](12) NULL,
...
SELECT ntt.Groupe,ntt.Dateinventaire,ntt.Compte,ntt.ISIN_Ptf,ntt.Libelle_Ptf,ntt.code_Titre,ntt.isin_titre,ntt.Libelle_Titre,ntt.Valeur_Boursiere,
	   ...
	   '' AS 'Isin_origine_Niv_5', NULL AS 'pct_det_Niv_5',NULL AS 'Isin_proxy_Niv_5',
...
SELECT po.Groupe,po.Dateinventaire,po.Compte,po.ISIN_Ptf,po.Libelle_Ptf,
	   ...
	   isnull(po.Isin_proxy_Niv_5,po.isin_titre) AS 'Isin_origine_Niv_5',  -- isin du fonds source (si proxy.OPCVM, c est le fonds initial)
	   CASE WHEN an.AN > 0 THEN po.pct_det_Niv_3 * ( fga.Valeur_Boursiere + fga.Coupon_Couru) / an.AN-- poids du fonds Source dans le mandat si c'est un fond
					  ELSE NULL END AS 'pct_det_Niv_5',  -- si ce n'est pas un fonds FGA : vide
       CASE WHEN po.Isin_proxy_Niv_5 is not null THEN po.isin_titre
                                                 ELSE NULL END AS 'Isin_proxy_Niv_5',   -- isin du fonds utilisé en proxy (OPCVM FGA)	   
	   @niveau AS 'Numero_Niveau',
	   CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
	   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
			WHEN ta.Types IS NOT NULL AND ta.Types <> 'AS_PTF' THEN ta.Types -- transco pour les actifs sauf cash et monétaire
...
INTO #Lignes_OPCVM  -- niveau 5
FROM  #OPCVM_FGA po -- niveau 4
...
CASE WHEN p.code_Titre = 'Liquidité(OPCVM)'  THEN 'Cash OPCVM' ELSE p.code_Titre END AS 'isin_titre',p.Libelle_Titre,
		...
		left(po.isin_titre,12) AS 'Isin_origine_Niv_5', -- isin du fonds source
		...
		ELSE NULL END AS 'pct_det_Niv_5',  -- si ce n'est pas un fonds FGA : vide        
		left(po.Isin_proxy_Niv_5,12) AS 'Isin_proxy_Niv_5',   -- code du proxy utilisé
		@niveau AS 'Numero_Niveau',
		CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
	   CASE WHEN co.Types IS NOT NULL AND co.Types <> 'AS_PTF' THEN co.Types -- transco pour OPCVM
			WHEN ta.Types IS NOT NULL AND ta.Types <> 'AS_PTF' THEN ta.Types -- transco pour les actifs sauf cash et monétaire
...
FROM  #PROXY po  -- niveau 3
	LEFT OUTER JOIN PTF_PROXY p ON p.code_proxy = po.Isin_proxy_Niv_5 AND p.Date= @date
...
SELECT [Groupe],[Dateinventaire],[Compte],[ISIN_Ptf],[Libelle_Ptf],[code_Titre],[isin_titre]
           ...
           --,case 
           --when [Isin_origine_Niv_3] ='' then 'N5'+SUBSTRING([Isin_origine_Niv_5],3,12)
           --when [Isin_origine_Niv_5] ='' then 'N4'+SUBSTRING([Isin_origine_Niv_3],3,12)
           ----else SUBSTRING([Isin_origine_Niv_3],1,6)+SUBSTRING([Isin_origine_Niv_5],1,6) end as [Isin_origine_Niv_3],
           --else 'N5'+SUBSTRING([Isin_origine_Niv_5],3,12) end as [Isin_origine_Niv_3],
           ,ROW_NUMBER ( ) OVER ( ORDER BY Isin_origine_Niv_3,Isin_origine_Niv_5) as [Isin_origine_Niv_3],
           [pct_det_Niv_5] as [pct_det_Niv_3],
           [Isin_proxy_Niv_5] as [Isin_proxy_Niv_3]
