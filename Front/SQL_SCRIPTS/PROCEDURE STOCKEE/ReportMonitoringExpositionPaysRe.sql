...
/****** Object:  StoredProcedure [dbo].[ReportMonitoringExpositionPaysRe]    Script Date: 09/18/2012 10:07:14 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[ReportMonitoringExpositionPaysRe]
...
BEGIN
  set @cleReport = 'MonitorExpoPaysRe'
END
...
-- calcul de l encours par zone
 ...
 select  rapport.Groupe as groupe,		 
		 ...
		 'ZONE_'+convert(varchar,rapport.numeroZone)+'_GLOBAL' ,
  		 'RENDEMENT pour la Zone' as libelle,
...
/************************************************************************/
 ...
 create table #rendement(groupe varchar (25)COLLATE DATABASE_DEFAULT ,
                                    pays varchar(30)COLLATE DATABASE_DEFAULT ,
                                    libelle varchar(60)COLLATE DATABASE_DEFAULT ,
                                    valeur float)
                                    
                                    
 insert into #rendement (groupe, pays, libelle, valeur)
 select  rapport.Groupe as groupe,		 
		 rapport.pays as pays,
		 rapport.libelle as sousSecteur,
         SUM( ( Valeur_Boursiere + Coupon_Couru ) * Rendement ) as valeur
  from (select * from #rapport_pays, #Groupe) as rapport 
  left outer join [dbo].[PTF_TRANSPARISE] as tr 
  on tr.Groupe = rapport.Groupe
  and tr.Dateinventaire = @dateInventaire
...
/*********************************************************/
  ...
  left outer join #rendement as rapport on rapport.groupe IN (select composant.groupeComposant from #GroupeCalculeComposant as composant where composant.groupeCalcule = gr.groupe)
...
from #encours as e
  left outer join #rendement as d on d.groupe = e.groupe and d.pays = e.Pays and d.libelle = e.libelle
