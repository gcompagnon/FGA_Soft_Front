/** configuration du rapport SupervisionGroupe en Duration Pondérée:
...
ALTER PROCEDURE ReportMonitoringGroupeDuration 
...
BEGIN
  set @cleReport = 'MonitoringGroupeDuration'
END

/** Parametres: ---FIN--------------------------------**/
-- Pre condition ------------------------------------
--executer le report ReportMonitoringGroupe  pour avoir les encours (denominateur de la formule)
-- lancer l extraction si besoin
declare @nb int
set @nb = (Select count(*) from PTF_RAPPORT where cle = @cleReport and date = @dateInventaire)

declare @returnCode as int
IF @nb = 0 
BEGIN
--Retour de 0 si OK
exec @returnCode = ReportMonitoringGroupe @dateInventaire,@niveauInventaire, @cleReport OUTPUT
END 
...
/************************************************************************/
 create table #duration(groupe varchar (25) COLLATE DATABASE_DEFAULT,ordreGroupe tinyint,rubrique varchar(120) COLLATE DATABASE_DEFAULT,
...
-- DURATION non significative  sur l ensemble des encours  
  ...
  select gr.groupe as groupe,
         ...
         SUM( ( Valeur_Boursiere + Coupon_Couru ) * Duration )
  from  #groupe  as gr 
...
insert 
  ...
  left outer join #duration as rapport 
 ...
 insert 
  ...
  update rapportZone1
   set valeur = rapportZone1.valeur - rapportZone2.valeur
    from #duration  as rapportZone1 
   left outer join #duration  as rapportZone2 
...
/**********************************************/
  ...
  select gr.groupe as groupe,
		 ...
		 rapport.ordre,
         SUM( rapport.valeur ) as valeur
  from #GroupeCalcule as gr, #duration as rapport 
