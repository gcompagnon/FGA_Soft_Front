...
/****** Object:  StoredProcedure [dbo].[ReportMonitoringRatingRe]    Script Date: 09/18/2012 10:10:08 ******/
SET ANSI_NULLS ON
GO
...
ALTER PROCEDURE [dbo].[ReportMonitoringRatingRe] 
...
BEGIN
  set @cleReport = 'MonitoringRatingRe'
END
...
create table #rendement(groupe varchar (25) COLLATE DATABASE_DEFAULT, rating varchar(15) COLLATE DATABASE_DEFAULT, rend float)
insert into #rendement
SELECT gr.groupe as groupe,
		gr.rating as rating,
		SUM((valeur_boursiere+coupon_couru)*rendement) as rend
	FROM #groupe_rating as gr
	left outer join [dbo].[PTF_TRANSPARISE] as tr 
	on tr.Groupe = gr.Groupe
	and tr.Dateinventaire = @dateInventaire
...
SELECT @cleReport as cle,
		...
		r.rend/e.encours as valeur
	FROM #groupe_rating as gr
	join #rendement as r
		ON r.groupe=gr.groupe AND r.rating = gr.rating
	join #encours as e
...
SELECT @cleReport as cle,
		...
		SUM(r.rend)/SUM(e.encours) as valeur
	FROM #groupe_rating as gm
	join #rendement as r
		ON r.groupe=gm.groupe AND r.rating = gm.rating
	join #encours as e
...
SELECT @cleReport as cle,
		...
		gr.ordreGroupe,
		@dateInventaire as dateInventaire,
		'RENDEMENT - Total' as rubrique,
		'RENDEMENT - Total' as libelle,
		0 as classementRubrique,
		SUM(R.rend)/SUM(e.encours) as valeur
	FROM #groupe_rating as gr
	join #rendement as r
		ON r.groupe=gr.groupe
	join #encours as e
...
SELECT @cleReport as cle,
	...
	FROM #groupe_rating as gm
	join #rendement as r
		ON r.groupe in ('MM AGIRC','MM ARRCO') 
	join #encours as e
...
SELECT @cleReport as cle,
	...
	FROM #groupe_rating as gm
	join #rendement as r
		ON r.groupe in ('MMP','INPR','CAPREVAL','CMAV','MM MUTUELLE','SAPREM','AUXIA','QUATREM','AUTRES')
	join #encours as e
...
SELECT @cleReport as cle,
	...
	FROM #groupe_rating as gm
	join #rendement as r
		ON r.groupe in ('ARCELOR MITTAL France','IDENTITES MUTUELLE','IRCEM MUTUELLE','IRCEM PREVOYANCE','IRCEM RETRAITE','UNMI')
