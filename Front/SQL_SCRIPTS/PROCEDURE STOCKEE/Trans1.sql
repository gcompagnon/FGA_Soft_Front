...
--DROP PROCEDURE Trans1

ALTER PROCEDURE Trans1
       @date DATETIME
AS 
...
--<DEBUG
--Certains FOnds OPCVM FGA ne sont pas transparisé.
--select distinct Compte, Isin_Ptf,Libelle_Ptf from PTF_FGA WHERE Groupe = 'OPCVM'  AND Dateinventaire= '28/02/2013'
--Les lignes en 
--select * from PTF_PARAM_PROXY where Date = '27/03/2013'
--select count (*) from PTF_TRANSPARISE where DateInventaire = '27/03/2013' and numero_niveau = 4
--DEBUG>
...
SELECT (Valeur_Boursiere+ Coupon_Couru ) / an.AN as 'pct_det_Niv_0',
	   pp.isin_titre AS 'Isin_proxy_Niv_1', --le code du fond à la source
...
LEFT OUTER JOIN PTF_AN an ON an.isin_Ptf = pp.code_proxy AND an.date= @date -- Pour récupérer l ActifNet du fonds
where pp.Source = 'OPCVM' AND  pp.Isin_Titre = ma.isin_titre and pp.date= @date

--4.{Lignes transparisables OPCVM_FGA}
INSERT INTO #OPCVM_FGA
SELECT (Valeur_Boursiere+ Coupon_Couru ) / an.AN as 'pct_det_Niv_0',
	   NULL AS 'Isin_proxy_Niv_1',
	   Groupe,Dateinventaire,ma.Compte,ma.ISIN_Ptf,ma.Libelle_Ptf,
	   code_Titre,isin_titre,Libelle_Titre,Valeur_Boursiere,
	   Coupon_Couru,Valeur_Comptable,Coupon_Couru_Comptable,PMV,PMV_CC,Type_Produit,Devise_Titre,Secteur,Sous_Secteur,
	   Pays,Emetteur,Rating,Grp_Emetteur,maturite,duration,ma.sensibilite,coursclose,quantite,ma.coupon,ma.rendement
FROM  #MANDAT ma 
LEFT OUTER JOIN PTF_AN an ON an.isin_Ptf = ma.isin_titre AND an.date= @date -- Pour récupérer l ActifNet du fonds
WHERE ma.Isin_Titre in (SELECT DISTINCT isin_ptf FROM #OPCVM)

--5.{Lignes transparisable Proxy type.PROXY : fonds externes}
SELECT NULL as 'pct_det_Niv_0', -- nous n avons pas l'actif net du fonds externes
	   pp.code_proxy AS 'Isin_proxy_Niv_1',
	   Groupe,Dateinventaire,ma.Compte,ma.ISIN_Ptf,ma.Libelle_Ptf,
...
SELECT po.Groupe,po.Dateinventaire,po.Compte,po.ISIN_Ptf,po.Libelle_Ptf,
	   ...
	   po.pct_det_Niv_0 * fga.Valeur_Boursiere As 'Valeur_Boursiere',
	   po.pct_det_Niv_0 * fga.Coupon_Couru AS 'Coupon_Couru',
	   po.pct_det_Niv_0 * fga.Valeur_Comptable AS 'Valeur_Comptable',
	   po.pct_det_Niv_0 * fga.Coupon_Couru_Comptable AS 'Coupon_Couru_Comptable',
	   po.pct_det_Niv_0 * fga.PMV AS 'PMV',
	   po.pct_det_Niv_0 * fga.PMV_CC AS 'PMV_CC',
	   ...
	   po.pct_det_Niv_0 * fga.quantite AS quantite,
	   po.pct_det_Niv_0 AS 'pct_det_Niv_0',
	   isnull(po.Isin_proxy_Niv_1,po.isin_titre) AS 'Isin_origine_Niv_1',  -- isin du fonds source (si proxy.OPCVM, c est le fonds initial)
	   CASE WHEN an.AN > 0 THEN po.pct_det_Niv_0 * ( fga.Valeur_Boursiere + fga.Coupon_Couru) / an.AN-- poids du fonds Source dans le mandat si c'est un fond
					  ELSE NULL END AS 'pct_det_Niv_1',  -- si ce n'est pas un fonds FGA : vide
       CASE WHEN po.Isin_proxy_Niv_1 is not null THEN po.isin_titre
                                                 ELSE NULL END AS 'Isin_proxy_Niv_1',   -- isin du fonds utilisé en proxy (OPCVM FGA)
	   '' AS 'Isin_origine_Niv_2', NULL AS 'pct_det_Niv_2', NULL AS 'Isin_proxy_Niv_2',
  	   '' AS 'Isin_origine_Niv_3', NULL AS 'pct_det_Niv_3', NULL AS 'Isin_proxy_Niv_3',
	   1 AS 'Numero_Niveau',
	   CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
...
INTO #Lignes_OPCVM  -- niveau 1
FROM  #OPCVM_FGA po -- niveau 0
...
CASE WHEN p.code_Titre = 'Liquidité(OPCVM)'  THEN 'Cash OPCVM' ELSE p.code_Titre END AS 'isin_titre',p.Libelle_Titre,
		...
		CASE WHEN p.code_Titre = 'Liquidité(OPCVM)'  THEN 1 ELSE 0 END AS 'pct_det_Niv_0',
		left(po.isin_titre,12) AS 'Isin_origine_Niv_1', -- isin du fonds source
		...
		ELSE NULL END AS 'pct_det_Niv_1',  -- si ce n'est pas un fonds FGA : vide        
		left(po.Isin_proxy_Niv_1,12) AS 'Isin_proxy_Niv_1',   -- code du proxy utilisé
		'' AS 'Isin_origine_Niv_2',	NULL AS 'pct_det_Niv_2',NULL AS 'Isin_proxy_Niv_2',
		'' AS 'Isin_origine_Niv_3',	NULL AS 'pct_det_Niv_3',NULL AS 'Isin_proxy_Niv_3',
		1 AS 'Numero_Niveau',
		CASE WHEN co.Pays IS NULL THEN zg.Zone ELSE co.Pays END AS 'Zone_Géo',
...
INTO #Lignes_PROXY --Niveau1
FROM  #PROXY po  -- niveau 0
	LEFT OUTER JOIN PTF_PROXY p ON p.code_proxy = po.Isin_proxy_Niv_1 AND p.Date= @date
