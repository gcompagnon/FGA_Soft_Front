...
set @nb = (Select count(*) from PTF_RAPPORT_NIV2 where date = @dateInventaire and Groupe ='FGA' and cle = 'MonitorExpoPaysRe' )
IF @nb = 0 or ( ( @forcage <> 'N' OR @forcage <> 'n' ) and  @forcage <> '')
BEGIN
print 'MonitorExpoPaysRe'
execute ReportMonitoringExpositionPaysRe   @dateInventaire , 4 , 'MonitorExpoPaysRe'
END
