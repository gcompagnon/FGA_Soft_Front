...
set @nb = (Select count(*) from PTF_RAPPORT where date = @dateInventaire and Groupe ='FGA' and cle = 'MonitoringMaturite' )
IF @nb = 0 or ( ( @forcage <> 'N' OR @forcage <> 'n' ) and  @forcage <> '')
BEGIN
print 'MonitoringMaturite'
execute ReportMonitoringMaturite   @dateInventaire , 4 , 'MonitoringMaturite'
END
