...
set @nb = (Select count(*) from PTF_RAPPORT where date = @dateInventaire and Groupe ='FGA' and cle = 'MonitorGroupeCompta' )
IF @nb = 0 or ( ( @forcage <> 'N' OR @forcage <> 'n' ) and  @forcage <> '')
BEGIN
print 'MonitorGroupeCompta'
execute ReportMonitoringGroupeVNC    @dateInventaire , 4 , 'MonitorGroupeCompta'
END
