...
set @nb = (Select count(*) from PTF_RAPPORT where date = @dateInventaire and Groupe ='FGA' and cle = 'MonitoringRating' )
IF @nb = 0 or ( ( @forcage <> 'N' OR @forcage <> 'n' ) and  @forcage <> '')
BEGIN
print 'MonitoringRating'
execute ReportMonitoringRating   @dateInventaire , 4 , 'MonitoringRating'
END
