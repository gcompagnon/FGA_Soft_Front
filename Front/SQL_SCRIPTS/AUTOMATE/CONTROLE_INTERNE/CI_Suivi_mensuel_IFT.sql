--controle interne: suivi trimestriel des IFT
...
and 
   ( left(P._codedutype,2) in ('FT','FA','FD') or P._codedutype in ('CALL','PUT')) )   
...
(select
    ...
    O._compte,
    C._libellecli,
     convert(varchar,O._dateheuresaisie,103),
     convert(varchar,O._dateoperation,103) as date_operation,
     convert(varchar,O._datevaleur,103),
     O._sens,
     O._codedevise,
     O._contredevise,
     O._terme,
     ...
     '',
     '',
     isnull(O._commentaire,''),
     '',
     '' 
from
      fcp.change O
      left outer join fcp.cpartfcp C on O._compte=C._compte
      left outer join com.courtier co on O._codecourt=co._codecourt
      left outer join com.operater T1 on O._codeoperateur=T1._codeoperat
